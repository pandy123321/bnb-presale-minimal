...
contract Pangu2IntegrationTest is Test {
    ...
    function setUp() public virtual {
        ...
        supportPool = new SupportPool(
            address(token), address(wbnb), address(adapter), address(oracle), 300, 5 minutes, address(this), EMERGENCY
        );
        ...
        token.setSystemTransferContext(
            address(liquidityManager), TransferContext.Kind.LIQUIDITY_WITHDRAWAL, true
        );
        token.setSystemTransferContext(
            address(liquidityManager), TransferContext.Kind.SYSTEM_CREDIT_UNKNOWN, true
        );
        token.setSystemTransferContext(
    ...
    function _approveAndPublishEpoch(
        uint256 epochId,
        bytes32 root,
        uint256 totalAmount,
