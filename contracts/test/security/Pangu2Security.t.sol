...
contract Pangu2SecurityTest is Pangu2IntegrationTest {
    ...
    function testProfitTaxUsesWholeSellAmount() public view {
        ...
        uint256 incorrectSequentialExtraTax = ((sellAmount - 400) * 600) / 10_000;
        assertEq(incorrectSequentialExtraTax, 576);
        assertEq(support + burn - 400, 600);
    }
    ...
    function testBuybackIntervalCannotBeBypassed() public {
        vm.prank(USER);
        tradeRouter.buy{value: 10 ether}(9 ether, block.timestamp + 5 minutes);
        vm.startPrank(USER);
        token.approve(address(tradeRouter), 1 ether);
        tradeRouter.sell(1 ether, 0.9 ether, block.timestamp + 5 minutes);
        vm.stopPrank();

        vm.prank(KEEPER);
        feeVault.convertSupport(0.04 ether, 0.03 ether, block.timestamp + 5 minutes);
        supportPool.buyback();
    ...
    function testExpiredDividendRemainderBecomesCarryOnly() public {
        ...
        assertEq(token.balanceOf(address(distributor)), 0.9 ether);
    }

    function testCostBasisSystemRegistryCannotBeChangedDirectlyByGovernance() public {
        vm.expectRevert();
        costBasis.setSystemAddress(RECIPIENT, true);
    }

    function testCoreSystemContractsCannotBeRemovedFromTokenRegistry() public {
    ...
    function testFeeVaultCreditRejectsNonTokenCaller() public {
        vm.expectRevert();
