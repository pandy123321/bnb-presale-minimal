...
contract BuybackLockerTest is Test {
    ...
    function testFixedDurationRejectsEarlyAndReleasesOnlyToImmutableRecipient() public {
        ...
        assertTrue(locker.isSolvent());

        vm.expectRevert(BuybackLocker.BatchNotLocked.selector);
        locker.release(batchId);
    }

    function testConstructorRejectsInvalidModeDurationPairs() public {
        vm.expectRevert(BuybackLocker.InvalidConfiguration.selector);
        new BuybackLocker(
            address(token), address(supportPool), BuybackLocker.LockMode.PERMANENT, 1, RECIPIENT
        );

        vm.expectRevert(BuybackLocker.InvalidConfiguration.selector);
        new BuybackLocker(
            address(token), address(supportPool), BuybackLocker.LockMode.FIXED_DURATION, 0, RECIPIENT
        );
    }
