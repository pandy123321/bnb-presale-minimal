...
实现 Agent可以运行本地 `forge fmt/build/test`、Fuzz 和 Invariant 作为代码验证；不得运行 `forge script`、Fork、RPC、`cast send/call`、Anvil 或任何部署命令。部署脚本默认只读；除本文件 §7 明确规定、经 Review Adjudication 并取得用户批准的 `COMPILE_COMPATIBILITY_EXCEPTION` 外，不得修改。该例外只处理编译期类型、接口或 constructor 参数位置兼容，不授权修改地址、权限策略、部署顺序、迁移逻辑，也不构成部署审核或部署批准。
