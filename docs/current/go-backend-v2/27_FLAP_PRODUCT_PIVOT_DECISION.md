...
F0_DOCUMENTS_COMPLETE = V6_ECONOMIC_CHANGE_FIX_READY
...
V5 的内容审核通过只证明上一版模型，不授权本次经济变更。只有 V6 独立审核 `APPROVED`、执行方二次裁决 `ACCEPTED`、责任人对 V6 冻结 `SIGNED` 后，才允许设置 `F1_ENTRY_AUTHORIZED = YES`。独立审核 `APPROVED` 单独只允许进入 Responsible Owner Freeze，不授权 F1。
