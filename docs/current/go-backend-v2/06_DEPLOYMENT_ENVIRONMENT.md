...
角色和对象权限以 [sql/0002_binggoplus_v2_runtime_privileges.sql](./sql/0002_binggoplus_v2_runtime_privileges.sql) 为机器规范。由于 Epoch/Command Trigger 以 `current_user` 强制职责边界，运行进程必须**直接使用同名 LOGIN Role**：`bgp_api`、`bgp_indexer`、`bgp_projector`、`bgp_dividend`、`bgp_reconciler`、`bgp_auditor`、`bgp_readonly`。不采用 Login Role + INHERIT Group Role 模式，也不得依赖隐式继承满足 Trigger 身份；如未来改为 Group Role 模型，必须通过独立 Freeze 修订、明确 `SET ROLE`、membership、NOINHERIT 和连接初始化验收后方可实施。`bgp_migrator` 为独立受控 LOGIN Role，仅发布流程使用。平台预先创建 Login Role、Database `CONNECT` 和 Secret；仓库脚本不创建密码。运行时角色不得拥有 Superuser/CreateRole/CreateDB/BypassRLS，不得拥有 Schema、继承 `bgp_migrator` 或获得切换到迁移角色的能力。
...
Role 验收还必须证明每个进程连接执行 `SELECT current_user` 时恰等于其批准角色名；不得只验证 `session_user`、membership 或有效 GRANT。任何 `current_user` 不匹配均使对应进程 Ready=false，且不得写入业务状态。
