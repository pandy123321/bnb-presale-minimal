...
> 状态：`HISTORICAL_PANGU2_RUNTIME_GATE / NOT_CURRENT_FLAP_STAGE_AUTHORITY`。本文件保留 PANGU2 Go V2 Runtime Gate 的原始状态和证据，不授权恢复旧 G2-G9 主线。当前产品与阶段权威为 `27_FLAP_PRODUCT_PIVOT_DECISION.md` 和 `30_FLAP_F0_F11_EXECUTION_PLAN.md`：`PRODUCT_MAINLINE = FLAP`、`CURRENT_STAGE = FLAP-F0`、`F1_ENTRY_AUTHORIZED = NO`。
