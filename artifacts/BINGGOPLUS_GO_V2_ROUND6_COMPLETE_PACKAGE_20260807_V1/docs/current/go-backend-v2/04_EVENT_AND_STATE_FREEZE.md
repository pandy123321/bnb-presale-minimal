...
- Preflight 通过复合外键精确绑定 Artifact 的 content hash、root、amount、`input_sha256` 与 `projector_manifest_sha256`；
...
- `bgp_projector` 消费 `DividendRootPublished / EpochClosed / EpochCancelled`，对 event-derived 字段做列级更新：`state/merkle_root/claim_start/claim_end/carry_raw/updated_at`；
- 数据库 Trigger 阻止 Builder 覆盖 Projector 拥有的后发布状态，也阻止 Projector 回写发布前状态。
