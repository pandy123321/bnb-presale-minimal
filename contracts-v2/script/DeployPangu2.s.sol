...
///         网络地址从环境变量读取（EXPECTED_CHAIN_ID, EXPECTED_WBNB, EXPECTED_FACTORY, EXPECTED_ROUTER）。
///         TradeRouter 默认为暂停状态。
contract DeployPangu2 is Script {
    ...
    function run() external {
        ...
        address WBNB = vm.envAddress("EXPECTED_WBNB");
        address FACTORY = vm.envAddress("EXPECTED_FACTORY");
        address ROUTER = vm.envAddress("EXPECTED_ROUTER");
        ...
        // TradeRouter 默认暂停 — Governance 在 Finalize 后手动解停
        tradeRouter.pause();
        ...
        // 不带 UNPAUSER 的合约（CostBasis, Adapter）
        {
            ...
            require(cb.hasRole(DA, governance), "costBasis grant DA");
            cb.grantRole(GOV, governance);
            require(cb.hasRole(GOV, governance), "costBasis grant GOV");
        }
        {
            AccessControl ad = AccessControl(address(adapter));
            ad.grantRole(DA, governance);
            require(ad.hasRole(DA, governance), "adapter grant DA");
            ad.grantRole(GOV, governance);
            require(ad.hasRole(GOV, governance), "adapter grant GOV");
        }
        ...
        require(
            token.systemTransferContextAllowed(address(staking), TransferContext.Kind.STAKING_DEPOSIT),
            "staking missing DEPOSIT"
        );
        require(
            token.systemTransferContextAllowed(address(staking), TransferContext.Kind.STAKING_PRINCIPAL_RETURN),
            "staking missing PRINCIPAL_RETURN"
        );
        require(
            token.systemTransferContextAllowed(address(staking), TransferContext.Kind.STAKING_REWARD),
            "staking missing REWARD"
        );
