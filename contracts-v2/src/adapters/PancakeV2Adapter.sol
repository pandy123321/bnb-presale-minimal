...
contract PancakeV2Adapter is AccessControl, ReentrancyGuard, IPancakeV2Adapter {
    ...
    /// @notice Swap always delivers the output ERC20 token to recipient.
    ///         Adapter NEVER sends native BNB. Callers handle their own unwrap.
    function swapExactInput(
        ...
        // Always use swapExactTokensForTokens — both directions deliver ERC20 output
        uint256[] memory amounts =
            router.swapExactTokensForTokens(amountIn, amountOutMinimum, path, recipient, deadline);
        amountOut = amounts[1];
