...
PGBNB仓库已在 `pgbnb` 分支完成Web3 Hub结构：仓库根目录保存统一治理规则，所有Web3业务项目位于 `hub/`；原BNB Presale Minimal已归档至 `hub/pixiu1/`。人工项目负责人已经批准 `DR-P2-0001`，盘古2固定使用 `hub/pangu2/` 作为唯一项目根目录。
...
PB-S0 Repository Initialization Gate: GO
Product Development Gate: BLOCKED
Production Status: NO-GO
Automatic Merge: FORBIDDEN
Automatic Deployment: FORBIDDEN
```

PB-S0可以进入目录初始化、治理文件、工具链、LOCAL/CI和Schema候选建设；PB-S1至PB-S6仍受开放Decision、Spec SHA、Ruleset Bundle SHA和环境证据阻塞。

## 2. Document Control

| Field | Value |
|---|---|
| Project / Stage | `PANGU2 / PB-S0` |
| Document Version | `V1.1-RC` |
| Document Status | `BASELINE_CANDIDATE` |
| Document Owner | `PENDING` |
| Prepared By | Development Baseline Documentation Agent |
...
| Base Branch | `pgbnb` |
| Base SHA | `PENDING_P2_G02` |
| Project Root | `hub/pangu2/` |
| Archived Project | `hub/pixiu1/` (`READ_ONLY`) |
| Repository Decision | `DR-P2-0001 / ACTIVE` |
| Spec Path | `hub/pangu2/docs/baseline/PANGU2_DEVELOPMENT_BASELINE_V1.1_RC.md` |
| Spec SHA | `PENDING_P2_G02` |
| Ruleset Entry | `AGENTS.md` |
| Ruleset Bundle SHA | `PENDING_P2_G03` |
| Supersedes | V1.0候选稿与PR #3根目录候选路径 |
| Open Decisions | `DR-P2-0002` 至 `DR-P2-0010` |
| Effective Scope | PB-S0治理、仓库、环境和Schema基线建设 |
...
```text
Decision ID: DR-P2-0001
Status: ACTIVE
Selected Option: C — PGBNB Web3 Hub中的独立项目目录
Repository: pandy123321/bnb-presale-minimal
Base Branch: pgbnb
Project Root: hub/pangu2/
...
- 盘古2产品代码、迁移、配置、项目文档和证据只能进入 `hub/pangu2/`。
- `hub/pixiu1/**` 默认Forbidden；不得直接在归档项目上扩展盘古2。
- 仓库根治理路径只有在Task Spec明确授权时才能修改。
- 从pixiu1复用资产必须复制后重新验证，不能通过相对路径直接依赖归档代码。
- 原PR #3属于旧 `main + 根目录docs` 候选路径，状态为 `SUPERSEDED_BY_DR_P2_0001`，不得作为产品开发基线。
...
|---|---|---|---|---|
| DR-P2-0001 | Repository Strategy | ACTIVE / 人工负责人 | 已选C：pgbnb + hub/pangu2 | 已解除仓库阻塞 |
...
策略为 `PANGU2_FIRST_PLATFORM_READY`：先完成盘古2，只保留低成本边界，不建设多租户、多链、插件或微服务。固定仓库为 `pandy123321/bnb-presale-minimal`，Base Branch为 `pgbnb`，项目根目录为 `hub/pangu2/`。环境顺序为 `LOCAL → CI → BSC_TESTNET → STAGING → TESTNET_CLOSEOUT`；BSC_MAINNET 始终 `NO-GO`。项目协调 Agent 管 Gate；执行 Agent 实施固定 Task Spec；审核 Agent 只读审核固定 SHA；人工项目负责人批准重大范围、权限、合并和发布。
...
候选为 Laravel模块化单体 + TypeScript Chain Worker + Laravel Horizon Job Worker + Vue DApp/Admin + Foundry，第一阶段不拆微服务。所有模块位于 `hub/pangu2/`；禁止从运行时直接依赖 `hub/pixiu1/`。
...
PB-S0实际任务：`P2-G01`仓库Decision（已完成）、`G02`初始化hub/pangu2与迁移基线、`G03`PANGU2项目Ruleset、`G04`精确版本、`G05`LOCAL/CI、`G06`Actions、`G07`Secret/Forbidden/Schema检查、`G08`参数Decision、`G09`Schema候选、`G10`Closeout。
...
P2-G02候选字段：Repository=`pandy123321/bnb-presale-minimal`；Base Branch=`pgbnb`；Project Root=`hub/pangu2/`；Allowed仅项目初始化和基线迁移路径；Forbidden包含`hub/pixiu1/**`、产品代码、Workflow、Secret和主网配置；Open Decisions为0002..0010；Next Role为审核Agent。
...
| Condition | Current |
|---|---|
| DR-P2-0001 ACTIVE | YES |
| Repository / Base Branch | `pandy123321/bnb-presale-minimal` / `pgbnb` |
| Project Root | `hub/pangu2/`，待P2-G02初始化 |
| Approved Baseline | NO |
| Spec SHA / Ruleset Bundle SHA | PENDING / PENDING |
| Exact Versions/Lockfiles | NO |
| LOCAL/CI Implemented | NO |
| DR-P2-0002..0010 | PENDING |
| Mainnet NO-GO | YES |

```text
Project Start Gate: NEEDS_CONFIRMATION
PB-S0 Repository Initialization Gate: GO
PB-S1/PB-S2 Product Development Gate: BLOCKED
```

P2-G02可以执行；任何产品代码任务仍不得启动。
...
| V1.1-RC-PGBNB | BASELINE_CANDIDATE | DR-P2-0001生效；固定pgbnb、hub/pangu2和pixiu1归档边界 |
...
- [ ] P2-G02 PR保持Draft，且只修改Task Spec授权路径
- [ ] `hub/pixiu1/**`、产品代码、Workflow、Secret和主网配置未改
