...
import {LockDecision} from "../src/libraries/LockDecision.sol";
...
contract DeployPangu2 is Script {
    ...
    function _readAndValidateConfig() private returns (DeployConfig memory c) {
        ...
        uint256 rawLockMode = vm.envUint("LOCK_MODE");
        uint256 rawLockDuration = vm.envUint("LOCK_DURATION");
        c.lockDecisionId = vm.envString("LOCK_DECISION_ID");
        (c.lockMode, c.lockDuration) = LockDecision.validate(
            rawLockMode, rawLockDuration, c.releaseRecipient, c.lockDecisionId
        );
    ...
    function _writeManifest(Deployment memory d, DeployConfig memory c) private {
        ...
        vm.serializeUint(objectKey, "lockerMode", uint256(c.lockMode));
