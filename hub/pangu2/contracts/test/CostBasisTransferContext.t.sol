...
contract CostBasisTransferContextTest is Pangu2IntegrationTest {
    ...
    function testUserCannotTransferDirectlyToNonDepositSystemAddresses() public {
        ...
        address[7] memory forbidden = [address(tradeRouter), address(feeVault), address(supportPool), address(locker), address(distributor), address(adapter), address(liquidityManager)];
    ...
    function testFuzz_KnownPositionAlwaysMatchesActualBalanceAfterLiquidityRoundTrip(uint96 rawDeposit, uint96 rawReturn) public {
        _buy(USER, 10 ether);
        ...
        ICostBasisManager.Position memory p = costBasis.positionOf(USER);
        if (p.status == ICostBasisManager.PositionStatus.KNOWN) assertEq(p.trackedBalance, token.balanceOf(USER));
    }
