...
import {IPancakeV3Pool, IPancakeV3NonfungiblePositionManager, IWBNB} from "pangu2/interfaces/IPancakeV3.sol";
...
contract PancakeV3BscTestnetForkTest is Test {
    ...
    function setUp() public {
        ...
        address poolAddress = IPancakeV3NonfungiblePositionManager(POSITION_MANAGER).createAndInitializePoolIfNecessary(token0, token1, FEE_TIER, uint160(uint256(1) << 96));
        pool = IPancakeV3Pool(poolAddress);
        adapter = new PancakeV3Adapter(address(token), WBNB, FACTORY, poolAddress, SWAP_ROUTER, QUOTER_V2, FEE_TIER, address(this));
        oracle = new PancakeV3TwapOracle(address(token), WBNB, FACTORY, poolAddress, FEE_TIER, 30 minutes, 16, 300, 1);
        ...
        token.setSystemTransferContext(address(liquidityGateway), TransferContext.Kind.LIQUIDITY_WITHDRAWAL, true);
        ...
        liquidityGateway.addLiquidity(Pangu2LiquidityGateway.AddLiquidityParams({tokenDesired: 1_000 ether, wbnbDesired: 1_000 ether, tokenMin: 900 ether, wbnbMin: 900 ether, tickLower: -60_000, tickUpper: 60_000, deadline: block.timestamp + 5 minutes}));
    ...
    function testRealPancakeV3CompleteProtocolFlow() public {
        ...
        vm.startPrank(USER); token.approve(address(tradeRouter), 1 ether); tradeRouter.sell(1 ether, 1, block.timestamp + 5 minutes); vm.stopPrank();
        ...
        vm.startPrank(PROFIT_USER); token.approve(address(tradeRouter), 1 ether); tradeRouter.sell(1 ether, 1, block.timestamp + 5 minutes); vm.stopPrank();
        ...
        vm.startPrank(UNKNOWN_USER); token.approve(address(tradeRouter), 1 ether); tradeRouter.sell(1 ether, 1, block.timestamp + 5 minutes); vm.stopPrank();
        ...
        IDividendDistributor.EpochCommitment memory c = IDividendDistributor.EpochCommitment({merkleRoot: root, artifactChecksum: keccak256("BSC_TESTNET_FORK_EPOCH_1"), totalAmount: epochTotal, claimStart: uint64(block.timestamp), claimEnd: uint64(block.timestamp + 30 days), snapshotBlock: uint32(block.number), schemaVersion: 1});
    ...
    function _writeThirtyMinuteOracleHistory(uint256 amountPerSwap) internal {
        ...
        for (uint256 i; i < 16; ++i) {
            ...
            adapter.swapExactInput(WBNB, address(token), amountPerSwap, 1, address(liquidityGateway), block.timestamp + 5 minutes);
        }
    ...
    function _movePriceAndRebuildTwap(uint256 amount) internal {
        adapter.swapExactInput(WBNB, address(token), amount, 1, address(liquidityGateway), block.timestamp + 5 minutes);
        _writeThirtyMinuteOracleHistory(0.000001 ether);
    }
}
