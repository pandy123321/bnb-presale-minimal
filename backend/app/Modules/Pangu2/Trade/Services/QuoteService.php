 ...
 *
 * Source determination (strict whitelist):
 *   - contract_preview → LIVE   (real on-chain contract call)
 *   - unavailable/error/mock   → UNAVAILABLE
 *
 * Current phase: mock-mode until ABI and RPC are available.
 * When a real contract call is implemented, set source to 'contract_preview'.
 *
 * Hard rules:
 *  - Never calculate tax rates client-side (frontend must NOT pass rate)
 *  - All amounts are integer decimal strings (not hex)
 *  - Notional rate/tax are NOT executable — actual execution happens on-chain
 *  - Buy is ALWAYS 4% tax (fixed by contract)
 *  - Sell tax rate (4% or 10%) is contract-determined
 */
final class QuoteService
{
    ...
    /**
     * Available quote sources and their data_status mapping.
     */
    private const QUOTE_SOURCE_STATUS = [
        'contract_preview' => 'LIVE',
        'mock'             => 'UNAVAILABLE',
    ...
    /**
     * Map a quote source to its data_status.
     */
    public function sourceStatus(string $source): string
    {
        return self::QUOTE_SOURCE_STATUS[$source] ?? 'UNAVAILABLE';
    }

    /**
     * Return a mock buy quote for given BNB amount (in wei decimal string).
     */
    ...
    /**
     * Return a mock sell quote for given token amount (raw decimal string).
     */
